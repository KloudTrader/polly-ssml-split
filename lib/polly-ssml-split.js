...
class PollySSMLSplit {
    ...
    split(ssml) {
        ...
        // console.log(traversalStack)
        while (traversalStack.length) {
            const node = traversalStack.pop()
            // console.log(node)
            if (node.type === "TEXT" || node.type === "break" || node.type === "amazon:breath") {
                flattenedTree.push(node)
            } else {
                // const newNode = traversalStack.pop()
                if (node.children) {
        ...
        flattenedTree.forEach(node => {
            ...
            while (currentNode.type !== 'root') {
                if (currentNode.type !== 'TEXT') {
                    // open tag
                    firstPartAcc = `<${currentNode.type}${currentNode.value}>` + firstPartAcc

                    // close tag
                    lastPartAcc += `</${currentNode.type}>` 
                }
            ...
            if (node.type === "TEXT") {
                ...
                //     })
                //     // console.log(node)
                //     // const splittedText = node.value.split(/.|,|\t|\n|\r| /)
                ...
                pollyTextSplit.split(node.value).forEach(text => {
                    const ssmlAcc = firstPartAcc + text + lastPartAcc
                    outputSSML.push(ssmlAcc)
                })
            } else {
                // breath and pause tags
                const ssmlAcc = firstPartAcc + `<${node.type}${node.value}/>` + lastPartAcc
                outputSSML.push(ssmlAcc)
            }


        })
        console.log(outputSSML)
        ...
        this[_root].children.forEach(node => {
            ...
            if (this[_textLength] < this[_softLimit]) {
                ...
                // console.log(node)
                throw new NotPossibleSplitError('Top level SSML tag appeared to be too long.')
            }
        })
        ...
        if (this[_textLength] !== 0) {
            if (this[_textLength] < this[_hardLimit]) {
                ...
                console.log(this[_textLength])
                console.log(this[_hardLimit])
                console.log(this[_makeSpeakBatch](this[_accumulatedSSML]))
                console.log(this[_makeSpeakBatch])
    ...
    */
    ...
    [_traverseNode](currentNode) {
        ...
        if (currentNode.children) {
            ...
            console.log([_noChildrenNodeToText])
            console.log(this[_noChildrenNodeToText](currentNode))
